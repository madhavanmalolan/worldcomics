...
export async function POST(request) {
  try {
    ...
    const scenesContract = new ethers.Contract(
      addresses.scenes,
      contracts.scenes.abi,
      provider
    );
    ...
    try {
      ...
      // Find the log from the Scenes contract
      const sceneLog = receipt.logs.find(log => 
        log.address.toLowerCase() === addresses.scenes.toLowerCase()
      );

      if (!sceneLog) {
        console.log('No log found from Scenes contract');
        return NextResponse.json(
          { error: 'No log found from Scenes contract' },
          { status: 400 }
        );
      }

      // Decode the log data
      const decodedLog = scenesContract.interface.parseLog({
        topics: sceneLog.topics,
        data: sceneLog.data
      });
      ...
      const tokenURI = await scenesContract.tokenURI(tokenId);
      ...
      if (metadata.name !== providedName || 
          metadata.image !== providedImage) {
